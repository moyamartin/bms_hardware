EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 6 8
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L bms_hardware-rescue:TPS54331D-Regulator_Switching U4
U 1 1 60036491
P 4700 1250
F 0 "U4" H 4700 1920 50  0000 C CNN
F 1 "TPS54331D" H 4700 1829 50  0000 C CNN
F 2 "Package_SO:SOIC-8_3.9x4.9mm_P1.27mm" H 4700 1250 50  0001 L BNN
F 3 "https://ar.mouser.com/ProductDetail/Texas-Instruments/TPS54331DR/?qs=QyYHynRG50rsSYcB%2Ftz4Yw%3D%3D" H 4700 1250 50  0001 L BNN
	1    4700 1250
	1    0    0    -1  
$EndComp
$Comp
L Diode:B340 D19
U 1 1 60048579
P 1650 1350
F 0 "D19" V 1696 1270 50  0000 R CNN
F 1 "B340" V 1605 1270 50  0000 R CNN
F 2 "Diode_SMD:D_SMC" H 1650 1175 50  0001 C CNN
F 3 "http://www.jameco.com/Jameco/Products/ProdDS/1538777.pdf" H 1650 1350 50  0001 C CNN
	1    1650 1350
	0    -1   -1   0   
$EndComp
$Comp
L Device:C C37
U 1 1 6004918C
P 2750 1400
F 0 "C37" H 2865 1446 50  0000 L CNN
F 1 "330nF" H 2865 1355 50  0000 L CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.18x1.45mm_HandSolder" H 2788 1250 50  0001 C CNN
F 3 "~" H 2750 1400 50  0001 C CNN
	1    2750 1400
	1    0    0    -1  
$EndComp
$Comp
L Device:C C38
U 1 1 60049307
P 3250 1400
F 0 "C38" H 3365 1446 50  0000 L CNN
F 1 "10uF" H 3365 1355 50  0000 L CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.18x1.45mm_HandSolder" H 3288 1250 50  0001 C CNN
F 3 "~" H 3250 1400 50  0001 C CNN
	1    3250 1400
	1    0    0    -1  
$EndComp
$Comp
L Device:C C42
U 1 1 600495AA
P 7450 1200
F 0 "C42" H 7565 1246 50  0000 L CNN
F 1 "10uF" H 7565 1155 50  0000 L CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.18x1.45mm_HandSolder" H 7488 1050 50  0001 C CNN
F 3 "~" H 7450 1200 50  0001 C CNN
	1    7450 1200
	1    0    0    -1  
$EndComp
$Comp
L Device:C C40
U 1 1 60049A2E
P 5400 1000
F 0 "C40" H 5515 1046 50  0000 L CNN
F 1 "2.2nF" H 5515 955 50  0000 L CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.18x1.45mm_HandSolder" H 5438 850 50  0001 C CNN
F 3 "~" H 5400 1000 50  0001 C CNN
	1    5400 1000
	1    0    0    -1  
$EndComp
$Comp
L Device:C C41
U 1 1 60049CF6
P 5800 1300
F 0 "C41" H 5915 1346 50  0000 L CNN
F 1 "1nF" H 5915 1255 50  0000 L CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.18x1.45mm_HandSolder" H 5838 1150 50  0001 C CNN
F 3 "~" H 5800 1300 50  0001 C CNN
	1    5800 1300
	1    0    0    -1  
$EndComp
$Comp
L Device:R R60
U 1 1 6004A339
P 3150 2850
F 0 "R60" H 3220 2896 50  0000 L CNN
F 1 "9K1" H 3220 2805 50  0000 L CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.20x1.40mm_HandSolder" V 3080 2850 50  0001 C CNN
F 3 "~" H 3150 2850 50  0001 C CNN
	1    3150 2850
	1    0    0    -1  
$EndComp
$Comp
L Device:R R61
U 1 1 6004A5D5
P 3150 3250
F 0 "R61" H 3220 3296 50  0000 L CNN
F 1 "1K8" H 3220 3205 50  0000 L CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.20x1.40mm_HandSolder" V 3080 3250 50  0001 C CNN
F 3 "~" H 3150 3250 50  0001 C CNN
	1    3150 3250
	1    0    0    -1  
$EndComp
$Comp
L Device:R R59
U 1 1 6004A934
P 750 3300
F 0 "R59" H 820 3346 50  0000 L CNN
F 1 "27K" H 820 3255 50  0000 L CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.20x1.40mm_HandSolder" V 680 3300 50  0001 C CNN
F 3 "~" H 750 3300 50  0001 C CNN
	1    750  3300
	1    0    0    -1  
$EndComp
$Comp
L Device:R R58
U 1 1 6004ABA6
P 750 2900
F 0 "R58" H 820 2946 50  0000 L CNN
F 1 "330K" H 820 2855 50  0000 L CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.20x1.40mm_HandSolder" V 680 2900 50  0001 C CNN
F 3 "~" H 750 2900 50  0001 C CNN
	1    750  2900
	1    0    0    -1  
$EndComp
$Comp
L Device:R R62
U 1 1 6004AED7
P 5400 1500
F 0 "R62" H 5470 1546 50  0000 L CNN
F 1 "5K1" H 5470 1455 50  0000 L CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.20x1.40mm_HandSolder" V 5330 1500 50  0001 C CNN
F 3 "~" H 5400 1500 50  0001 C CNN
	1    5400 1500
	1    0    0    -1  
$EndComp
$Comp
L Device:L L2
U 1 1 60057BAE
P 7200 950
F 0 "L2" V 7019 950 50  0000 C CNN
F 1 "6.8uH" V 7110 950 50  0000 C CNN
F 2 "Inductor_SMD:L_BOURNS_SRP6030" H 7200 950 50  0001 C CNN
F 3 "https://ar.mouser.com/ProductDetail/SRP6030VA-6R8M" H 7200 950 50  0001 C CNN
	1    7200 950 
	0    1    1    0   
$EndComp
Wire Wire Line
	3150 3000 3150 3050
Text GLabel 3200 3050 2    50   Input ~ 0
VSENSE
Wire Wire Line
	3200 3050 3150 3050
Connection ~ 3150 3050
Wire Wire Line
	3150 3050 3150 3100
Text GLabel 3900 1350 0    50   Input ~ 0
VSENSE
Wire Wire Line
	3900 1350 4000 1350
Text HLabel 3150 2650 1    50   Input ~ 0
V_OUT
Wire Wire Line
	3150 2650 3150 2700
Text HLabel 3150 3450 3    50   Input ~ 0
GND_BATT
Wire Wire Line
	3150 3450 3150 3400
Wire Wire Line
	750  3050 750  3100
Text HLabel 750  3500 3    50   Input ~ 0
GND_BATT
Wire Wire Line
	750  3450 750  3500
Text HLabel 750  2700 1    50   Input ~ 0
V_IN
Wire Wire Line
	750  2700 750  2750
Text GLabel 3900 950  0    50   Input ~ 0
EN
Wire Wire Line
	3900 950  4000 950 
Text GLabel 800  3100 2    50   Input ~ 0
EN
Wire Wire Line
	750  3100 800  3100
Connection ~ 750  3100
Wire Wire Line
	750  3100 750  3150
Wire Wire Line
	5400 1650 5600 1650
Text HLabel 5600 1750 3    50   Input ~ 0
GND_BATT
Wire Wire Line
	5600 1650 5600 1750
Connection ~ 5600 1650
Wire Wire Line
	5600 1650 5800 1650
Wire Wire Line
	5400 850  5800 850 
Connection ~ 5400 850 
Connection ~ 5400 1650
Wire Wire Line
	5400 1150 5400 1350
Connection ~ 3250 1250
Wire Wire Line
	3250 1250 4000 1250
Text HLabel 2700 1250 0    50   Input ~ 0
V_IN
Text HLabel 3250 1650 3    50   Input ~ 0
GND_BATT
Wire Wire Line
	3850 800  3850 850 
Wire Wire Line
	3850 850  4000 850 
Wire Wire Line
	3850 1100 3850 1050
Wire Wire Line
	3850 1050 4000 1050
Text GLabel 6950 950  0    50   Input ~ 0
PH
Wire Wire Line
	6950 950  7050 950 
Wire Wire Line
	7350 950  7450 950 
Wire Wire Line
	7450 950  7450 1050
Connection ~ 7450 950 
Text HLabel 7950 950  2    50   Input ~ 0
V_OUT
Text HLabel 7450 1450 3    50   Input ~ 0
GND_BATT
Wire Wire Line
	7450 1350 7450 1450
Wire Wire Line
	5800 850  5800 1150
Wire Wire Line
	5800 1450 5800 1650
Text Notes 1150 2400 0    50   Italic 10
EN - Adj Input Undervoltage Lockout (UVLO)
Text Notes 1150 2900 0    50   ~ 0
V_start = 16.29V
Text Notes 1900 2900 0    50   ~ 0
V_stop = 15.3V
Text Notes 1150 3000 0    50   ~ 0
R1 = (V_start - V_end)/I_hist = 330Kohm
Text Notes 1150 3100 0    50   ~ 0
R2 = (V_en)/(I_hist + 1uA) = 26836ohm
Wire Notes Line
	2950 2300 2950 3800
Wire Notes Line
	600  3800 600  2300
Text Notes 1150 3300 0    50   ~ 0
R1 = 330Kohm\nR2 = 27Kohm
Text Notes 3700 2400 0    50   Italic 10
V_out - Set point
Text Notes 3700 2850 0    50   ~ 0
V_out = 3v3\n
Text Notes 3700 2950 0    50   ~ 0
V_out = V_ref*(R5/R6 - 1)
Text Notes 3700 3200 0    50   ~ 0
R5 = 9K1\nR6 = 1K8\nV_OUT = 3V24\n
Wire Notes Line
	4850 2300 4850 3800
Wire Notes Line
	600  2300 4850 2300
Wire Notes Line
	600  3800 4850 3800
Wire Notes Line
	4600 2950 4600 2900
Wire Notes Line
	3150 1200 3150 1950
Wire Notes Line
	2400 1950 2400 1200
Wire Wire Line
	2700 1250 2750 1250
Connection ~ 2750 1250
Wire Wire Line
	3250 1550 2750 1550
Wire Wire Line
	2750 1250 3250 1250
Connection ~ 2750 1550
Text GLabel 1600 1100 0    50   Input ~ 0
PH
$Comp
L Device:C C39
U 1 1 60049780
P 3300 950
F 0 "C39" H 3415 996 50  0000 L CNN
F 1 "0.1uF" H 3415 905 50  0000 L CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.18x1.45mm_HandSolder" H 3338 800 50  0001 C CNN
F 3 "~" H 3300 950 50  0001 C CNN
	1    3300 950 
	1    0    0    -1  
$EndComp
$Comp
L Device:C C36
U 1 1 60056C6C
P 2050 1350
F 0 "C36" H 2165 1396 50  0000 L CNN
F 1 "3.3nF" H 2165 1305 50  0000 L CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.18x1.45mm_HandSolder" H 2088 1200 50  0001 C CNN
F 3 "~" H 2050 1350 50  0001 C CNN
	1    2050 1350
	1    0    0    -1  
$EndComp
Wire Wire Line
	2050 1150 2050 1200
Wire Wire Line
	2050 1150 4000 1150
Wire Wire Line
	2050 1500 2050 1550
Connection ~ 2050 1550
Wire Wire Line
	2050 1550 2750 1550
Wire Wire Line
	1600 1100 1650 1100
Wire Wire Line
	1650 1550 1650 1500
Wire Wire Line
	1650 1550 2050 1550
Wire Wire Line
	1650 1200 1650 1100
Connection ~ 1650 1100
Wire Wire Line
	1650 1100 3300 1100
Wire Notes Line
	1950 1200 1950 1950
Wire Notes Line
	1550 1200 1550 1950
Wire Notes Line
	1550 1950 3150 1950
Wire Notes Line
	1550 1200 3150 1200
Wire Wire Line
	3250 1650 3250 1550
Connection ~ 3250 1550
Text Notes 1550 1950 0    50   ~ 0
Catch \ndiode
Text Notes 1950 1950 0    50   ~ 0
Slow Start\n1.32mS
Text Notes 2400 1950 0    50   ~ 0
Bulk Capacitors\nDelta V_in = 1.1V\n
Wire Wire Line
	3300 800  3850 800 
Wire Wire Line
	3850 1100 3300 1100
Connection ~ 3300 1100
Wire Notes Line
	6600 650  11100 650 
Wire Notes Line
	11100 1700 6600 1700
Text Notes 6650 750  0    50   Italic 10
Output Filter
Text Notes 8300 800  0    50   ~ 0
L_min = (V_out * (Vin_max - Vout))/(Vin_max * K_ind * I_out * f_sw)
Text Notes 8300 900  0    50   ~ 0
L_min = 6.8uH
Wire Wire Line
	7450 950  7950 950 
Text Notes 8300 1050 0    50   ~ 0
Co_min = 1/(2*pi*F_CO(max))
Text Notes 8300 1150 0    50   ~ 0
Co_min = 6.875uF -> 10uF
Text Notes 8300 1250 0    50   ~ 0
ESR_max = 47.87mOhm
Wire Notes Line
	6600 650  6600 1700
Wire Notes Line
	11100 650  11100 1700
$EndSCHEMATC
